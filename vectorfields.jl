...
# ╔═╡ 01053090-c611-11ec-052e-63bc5bcc65ca
begin
using CairoMakie,LinearAlgebra
   
end

# ╔═╡ e975125e-d6fe-430f-bf22-3f502fd62859
begin
	range=-1:0.3:1
	ps = [Point3f(x, y, z) for x in range for y in range for z in range]
	f1(p)=p[1]*p[2]
	f2(p)=p[1]*p[2]^2*p[3]
	f3(p)=p[3]^3
    ns = map(p ->  Vec3f(f1(p),f2(p),f3(p)), ps)
	lengths = norm.(ns)
  ...
  arrows(
    ...
    color=lengths,
    lengthscale = 0.1,
	arrowsize = Vec3f(0.04, 0.04, 0.04),
    arrowcolor = lengths, linecolor = lengths,
    align = :center, axis=(type=Axis3,)
)
end

# ╔═╡ e9b7f9c1-1bf7-4ef0-b19b-0e77cafad306
let
	f = Figure(resolution = (800, 800))
	Axis(f[1, 1], backgroundcolor = "black")
	
	xs=ys = LinRange(-2, 2, 30)
	
	us = [2x for x in xs, y in ys]
	vs = [3y^2 * sin(y) for x in xs, y in ys]
...
# ╔═╡ 8705acbe-9a16-4605-bb8a-44215e876ba1
let
	xs=ys = LinRange(-2, 2, 30)
    zs = [x^2+y^3 for x in xs, y in ys]
	surface(xs, ys, zs, axis=(type=Axis3,))
end

# ╔═╡ 188c0e2e-0604-4400-8c54-579e6cc91b33
let
	ps = [Point(0,0) ]
    ns =  [Vec(2,2) ]
     arrows(
    ...
    linewidth = 0.5, arrowsize = Vec(0.3, 0.3,0.3),
    align = :center, axis=(type=Axis,
		)
)
end

# ╔═╡ ad7765b1-a845-43b5-b83b-81d8fc278b69
let
	f = Figure(resolution = (800, 800))
	Axis(f[1, 1], backgroundcolor = "black")
	
	xs=ys = LinRange(-4, 4, 20)
	
	us = [x for x in xs,y in ys]
	vs = [x/y for x in xs, y in ys]
	strength = vec(sqrt.(us .^ 2))
	
	arrows!(xs, ys, us, vs, arrowsize = 12, lengthscale = 0.1,
	    arrowcolor = strength, linecolor = strength)
	
	f
end

# ╔═╡ 7254705e-e7d9-4f0e-9176-1ddb8db3fb1e
let
	f = Figure(resolution = (800, 800))
	Axis(f[1, 1], backgroundcolor = "black")
	...
	us = [x for x in xs,y in ys]
	vs = [0.1*sqrt(y)+0.4*x^2 for x in xs, y in ys]
	strength = vec(sqrt.(us .^ 2+vs.^2))
...
# ╔═╡ 00a40ade-8a09-4b2f-8505-270fefdee58a
let
	f = Figure(resolution = (800, 800))
	Axis(f[1, 1], backgroundcolor = "black")
	...
	us = [-0.16*x+0.08*x*y for x in xs,y in ys]
	vs = [4.5*y-0.9*x*y for x in xs, y in ys]
	strength = vec(sqrt.(us .^ 2+vs.^2))
...
manifest_format = "2.0"
project_hash = "5000a45a8da24bf2bef2e6bac01de23872bd90f0"
